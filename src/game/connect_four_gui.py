...
from connect_four_logic import (
    ROW_COUNT, COLUMN_COUNT, create_board, drop_piece,
    is_valid_location, get_next_open_row, check_win, is_board_full
)
...
def draw_board(canvas, board):
    ...
    canvas.create_rectangle(0, SQUARE_SIZE, WIDTH, HEIGHT, fill=BOARD_COLOR, outline=BOARD_COLOR)
...
class ConnectFourApp:
    def __init__(self, master):
        ...
        # --- Status Label ---
        self.status_label = tk.Label(master, text=f"Player 1 ({PLAYER1_COLOR.capitalize()})'s Turn", 
                                     font=("Inter", 16, "bold"), fg="white", bg=BACKGROUND_COLOR)
        ...
        self.reset_button = tk.Button(master, text="Reset Game", command=self.reset_game, 
                                      font=("Inter", 12), bg="#e74c3c", fg="white", activebackground="#c0392b")
    ...
    def reset_game(self):
        ...
        self.game_over = False
        draw_board(self.canvas, self.board)
        self.status_label.config(text=f"Player 1 ({PLAYER1_COLOR.capitalize()})'s Turn", fg="white")
    ...
    def handle_motion(self, event):
        ...
        if 0 <= col < COLUMN_COUNT:
            ...
            if is_valid_location(self.board, col):
                self.indicator_circle = self.canvas.create_oval(x1, y1, x2, y2, fill=color, outline="#333", width=2, stipple="gray25")
            else:
    ...
    def handle_click(self, event):
        ...
        if is_valid_location(self.board, col):
            ...
            if check_win(self.board, self.turn):
                ...
                messagebox.showinfo("Game Over", f"Player {self.turn} ({winner_color.capitalize()}) is the winner!")
                ...
                self.status_label.config(text=f"Player {self.turn} ({next_player_color.capitalize()})'s Turn", fg="white")
